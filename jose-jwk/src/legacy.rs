//! Legacy JSON Web Key (JWK) implementation originally from the `elliptic-curve` crate.

#![cfg(feature = "legacy")]

// TODO(tarcieri): unify this with the other implementation when they reach feature parity.
...
{
    fn from(sk: &SecretKey<C>) -> JwkEcKey {
        let mut jwk = JwkEcKey::from(sk.public_key());
