...
pub trait SealingKey<'a> {
    ...
    fn seal(&'a self, alg: Sealing) -> Result<Self::Sealer, Self::StartError>;
}
...
pub trait Sealer: Update {
    ...
    fn finish(self, aad: impl AsRef<[u8]>) -> Result<Sealed, Self::FinishError>;
}
...
pub trait WrappingKey {
    ...
    fn wrap_cek(&self, sealer: &impl Sealer) -> Result<WrappedCek, Self::Error>;
}
